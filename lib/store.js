...
const DEFAULT_REDIS_DB_DIR = path.resolve(path.join(__dirname, '../var/db'))
const DEFAULT_REDIS_PID_PATH = path.resolve(path.join(__dirname, '../var/pid/redis.pid'))
...
var generateConfig = (config) => {
  ...
  if (config.dir) result.push(`dir ${path.resolve(path.join(__dirname, '..', config.dir))}`)
  if (config.pidfile) result.push(`pidfile ${path.resolve(path.join(__dirname, '..', config.pidfile))}`)
